...
void Energy_and_Force(Mat<int> face, Vertex3Layers vertex3, Vertex3Layers vertex3ref, Mat<int> one_ring_nodes, 
    ...
    double E_splayTilt = 0.0;      // splay-tilt energy
    ...
    if ( param.isBilayerModel == false ){
        ...
        cell_area_volume(face, vertex3.outlayer, param.isBoundaryFace, one_ring_nodes, param.GaussQuadratureN, elementSout, elementVout, gqcoeff, shape_functions, subMatrix, param.subDivideTimes);
    ...
    // three-layer mesh model    
        ...
        cell_area_volume(face, vertex3.inlayer, param.isBoundaryFace, one_ring_nodes, param.GaussQuadratureN, elementSin, elementVin, gqcoeff, shape_functions, subMatrix, param.subDivideTimes);
        ...
        double uv = param.uv;
        double V0out = param.V0out; 
        double Evolume = 0.0;// 0.5*uv/V0out * pow(Vout-V0out,2.0);
        /////////////////////////////////////////////////////////////////// 
        // for bending, height, splay-tilt, area and volume elasticity
        // for out-layer
    ...
    for ( int i = 0; i < vertex3.outlayer.n_rows; i++ ){
        ...
        rowvec direction(3); direction << 0.0 << 0.0 << 1.0 << endr; // planar surface, move along Z direction
...
double line_search_a_to_minimize_energy(double a0, Force3Layers& dx, Mat<int> face, Vertex3Layers vertex3, Vertex3Layers& vertex3ref, Mat<int> one_ring_nodes, 
    ...
    while ( isCriterionSatisfied == false ) {
        ...
        if ( toUpdateReference == false ){
            ...
            if ( E1 <= E0 + c1*a*shu0(0,0) && abs(shu1(0,0)) <= c2*abs(shu0(0,0)) ){ 
                break;
            }
            if ( a < 1.0e-9 ) {
...
void check_nodal_force(Mat<int> face, Vertex3Layers vertex3, Vertex3Layers vertex3ref, Mat<int> one_ring_nodes, 
  ...
  // outer layer 
  {
    ...
    for (int i =0; i<numvertexout; i++){
        ...
        if (i < param.isBoundaryVertex.n_cols && param.isBoundaryVertex(i) == 1) continue;

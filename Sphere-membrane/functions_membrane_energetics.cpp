...
void element_energy_force_regular(int whichLayer, bool isInsertionPatch, mat dots, Param param, double c0, double& Ebe, mat& F_be, mat& F_s, mat& F_v, rowvec gqcoeff, cube shape_functions, double a0) {
...
void element_energy_force_thickness_regular(bool isInsertionPatch, int whichLayer, mat dotsin, mat dotsout, Param param, double& E, mat& Fin, mat& Fout, rowvec gqcoeff, cube shape_functions){
    double h0 = param.thickness_in;
    double kt = param.Kthick_in/pow(h0,2.0);
    if ( whichLayer == 2 ){ // out-layer
    ...
    for (int i = 0; i < VWU.n_rows; i++) {
        ...
        if ( whichLayer == 2 ){ // out-layer: the norm is chosen on the out-monolayer
        ...
        }else if (whichLayer == 0){ // in-layer: the norm is chosen on the in-monolayer
...
void element_energy_force_thickness_irregularMinus(bool isInsertionPatch, int whichLayer, mat ori_dotsin, mat ori_dotsout, Param param, double& E, mat& cunchu1, mat& cunchu2, rowvec gqcoeff, cube shape_functions, SubMatrix subMatrix) {
    ...
    for (int j = 0; j < n; j++) {
        ...
        double e = 0.0;
        element_energy_force_thickness_regular(isInsertionPatch, whichLayer, dotsin, dotsout, param, e, f1, f2, gqcoeff, shape_functions);
        mat m1m = strans(M4*matrix);
        cunchu1 = cunchu1 + m1m*f1;
        cunchu2 = cunchu2 + m1m*f2;
        E = E + e;
...
void element_energy_force_thickness_irregularPlus(bool isInsertionPatch, int whichLayer, mat ori_dotsin, mat ori_dotsout, Param param, double& E, mat& cunchu1, mat& cunchu2, rowvec gqcoeff, cube shape_functions, SubMatrix subMatrix) {
    // cunchu1 is the inlayer force; cunchu2 is the outlayer force.
    // initialize the output parameters
    E = 0.0;
    cunchu1.fill(0);
...
void element_energy_force_thickness_pseudoRegular1(bool isInsertionPatch, int whichLayer, mat ori_dotsin, mat ori_dotsout, Param param, double& E, mat& cunchu1, mat& cunchu2, rowvec gqcoeff, cube shape_functions, SubMatrix subMatrix) {
    // cunchu1 is the inlayer force; cunchu2 is the outlayer force.
    // initialize the output parameters
    E = 0.0;
    cunchu1.fill(0);
...
void element_energy_force_thickness_pseudoRegular2(bool isInsertionPatch, int whichLayer, mat ori_dotsin, mat ori_dotsout, Param param, double& E, mat& cunchu1, mat& cunchu2, rowvec gqcoeff, cube shape_functions, SubMatrix subMatrix) {
    // cunchu1 is the inlayer force; cunchu2 is the outlayer force.
    // initialize the output parameters
    E = 0.0;
    cunchu1.fill(0);
...
void element_energy_force_thickness(bool isInsertionPatch, int whichLayer, Row<int> one_ring_nodes, Param param, double& Ethick, mat& fthick, mat vertex1, mat vertex2, rowvec gqcoeff, cube shape_functions, SubMatrix subMatrix){
    ...
    if ( one_ring_nodes(12) == -1 ) { 
        ...
        element_energy_force_thickness_regular(isInsertionPatch, whichLayer, dots1, dots2, param, ethick, fH1, fH2, gqcoeff, shape_functions);
        if ( whichLayer == 2 ){
    ...
    // irregular-minus patch
        ...
        element_energy_force_thickness_irregularMinus(isInsertionPatch, whichLayer, dots1, dots2, param, ethick, fH1, fH2, gqcoeff, shape_functions, subMatrix);
        if (  whichLayer == 2 ){
    ...
    // irregular-plus patch
        ...
        element_energy_force_thickness_irregularPlus(isInsertionPatch, whichLayer, dots1, dots2, param, ethick, fH1, fH2, gqcoeff, shape_functions, subMatrix);
        if ( whichLayer == 2 ){
    ...
    // pseudo-regular patch 1
        ...
        double ethick = 0.0; mat fH1(12,3); mat fH2(12,3); 
        element_energy_force_thickness_pseudoRegular1(isInsertionPatch, whichLayer, dots1, dots2, param, ethick, fH1, fH2, gqcoeff, shape_functions, subMatrix);
        if ( whichLayer == 2 ){
            for (int j = 0; j < 12; j++) { // mid-layer of mesh
                int nodenum = one_ring_nodes(j);
                fthick.row(nodenum + numvertex) = fthick.row(nodenum + numvertex) + fH1.row(j);
    ...
    // pseudo-regular patch 2
        ...
        double ethick = 0.0; mat fH1(12,3); mat fH2(12,3); 
        element_energy_force_thickness_pseudoRegular2(isInsertionPatch, whichLayer, dots1, dots2, param, ethick, fH1, fH2, gqcoeff, shape_functions, subMatrix);
        if ( whichLayer == 2 ){
            for (int j = 0; j < 12; j++) { // mid-layer of mesh
                int nodenum = one_ring_nodes(j);
                fthick.row(nodenum + numvertex) = fthick.row(nodenum + numvertex) + fH1.row(j);
...
void energy_force_regularization(int whichLayer, mat vertex, mat vertexold, Mat<int> face, Param param, Row<int> isinsertionpatch, double& E_regularization, mat& Fre, rowvec& deformnumbers){
    ...
    for (int i = 0; i < face.n_rows; i++){    
        if ( isinsertionpatch(i) == 1 && whichLayer == 2 ){
            continue;
        }
        ...
        /*
        if ( param.duringStepsToIncreaseInsertDepth == true ){ // force each face into regular triangle
            deformnumber_shape ++;
            double meansideold;
            double ktmp = k / 1.0; 
            //if ( param.isLocallyFinerFace[i] == true ){
            meansideold = sqrt( 4.0*area/sqrt(3.0) );
            //}else{
            //    meansideold = param.meanL;
            //}
...
void cell_area_volume(Mat<int> face, mat vertex, Mat<int> A, Row<int> Isinsertionpatch, int GaussQuadratureN, rowvec& element_area, rowvec& element_volume, rowvec gqcoeff, cube shape_functions, SubMatrix subMatrix, int n){ // A is face_ring_vertex 
    element_area.fill(0);
...
// here, A is the one-ring-nodes
double LocalAreaConstraintEnergy(int whichLayer, mat vertex, Mat<int> face, Mat<int> A, Param param, Row<int> Isinsertionpatch, rowvec gqcoeff, cube shape_functions, SubMatrix subMatrix, rowvec elementsqa){ // A is face_ring_vertex 
    double energy = 0.0;
    int GaussQuadratureN = param.GaussQuadratureN;
    mat VWU = setVMU(GaussQuadratureN); 
    double us;
...
rowvec determine_spontaneous_curvature(int whichLayer, Param param, Mat<int> face, mat vertex, Mat<int> insertionpatch){
...
rowvec calculate_thickness(int whichLayer, Mat<int> face, mat vertexin, mat vertexout, Mat<int> one_ring_nodes, Param param, cube shape_functions, SubMatrix subMatrix){ // A is face_ring_vertex 
